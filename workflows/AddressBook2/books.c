...
int Reload(int book)
{
    ...
    lenstr+=sprintf(str+lenstr, " ORDER BY Дата");
...
int AddItem ()
{
    ...
    char* SQL2 = "SELECT [Код контактов], Дата, Фамилия, Имя, Отчество, [Номер телефона], [Электронная почта], Адрес, Примечание, Фото \
...
void ShowBooks()
{
    ...
    gtk_window_set_title (GTK_WINDOW (wBooks), "Записная книга 2");
