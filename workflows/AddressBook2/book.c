...
static void DrawPageBook(GtkPrintOperation *op, GtkPrintContext *context, int page_nr)
{
    ...
    int day_data=gtk_spin_button_get_value_as_int (gDay);
    int month_data=gtk_spin_button_get_value_as_int (gMonth);
    int year_data=gtk_spin_button_get_value_as_int (gYear);
    ...
    strcpy(email_data, gtk_entry_get_text(GTK_ENTRY(gEmail)));
    ...
    // Выводим поле "Электронная почта"
    cairo_select_font_face(cr, "sans serif", CAIRO_FONT_SLANT_NORMAL, CAIRO_FONT_WEIGHT_NORMAL);
    cairo_move_to (cr, 10.0, 45.0);
    cairo_set_font_size (cr, 5.0);
    sprintf(buf, "Электронная почта - %s", email_data);
    cairo_show_text (cr, buf);
    cairo_stroke(cr);
...
int DrawPageBooks(GtkPrintOperation *op, GtkPrintContext *context, int page_nr)
{
    ...
    char* SQL = "SELECT [Код контактов], Дата, Фамилия, Имя, Отчество, [Номер телефона] FROM Контакты LIMIT ";
...
int LoadRecord(int book)
{
    ...
    char* SQL = "SELECT [Код контактов], Дата, Фамилия, Имя, Отчество, [Номер телефона], [Электронная почта], Адрес, Примечание, Фото \
    ...
    char buf[100];
    char buf2[100];
    sprintf(buf,"%s", sqlite3_column_text(stmt, C_DATE));

    // Получаем день
    memcpy( buf2, &buf[8], 2 );
    buf2[2] = '\0';
    gtk_spin_button_set_value(gDay, atoi(buf2));
    // Получаем месяц
    memcpy( buf2, &buf[5], 2 );
    buf2[2] = '\0';
    gtk_spin_button_set_value(gMonth,atoi(buf2));
    // Получаем год
    strncpy(buf2, buf, 4);
    buf2[4] = '\0';
    gtk_spin_button_set_value(gYear,atoi(buf2));
    ...
    // Электронная почта
    // Запись в виджеты
    if (sqlite3_column_text(stmt, C_EMAIL)>0)
    {
        sprintf(str, "%s",sqlite3_column_text(stmt, C_EMAIL));
    }
    else
    {
        sprintf(str, " ");
    }
    gtk_entry_set_text(GTK_ENTRY(gEmail),str);
...
void SaveRecord()
{
    ...
    // Готовим SQL-запрос
    lenstr = 0;
    lenstr+=sprintf(str, "UPDATE Контакты SET Дата =");
    lenstr+=sprintf(str+lenstr, "'%s', Фамилия ='%s', Имя ='%s', Отчество ='%s'", CurDate, last_name_data, name_data, otchestvo_data);
    lenstr+=sprintf(str+lenstr, ", [Номер телефона] ='%s', [Электронная почта] ='%s', Адрес ='%s', Примечание ='%s', Фото ='%s'", number_telefone_data, email_data, adres_data, primechanie_data, CurFoto);
...
void ShowBook(int book)
{
    ...
    gDay = GTK_SPIN_BUTTON(gtk_builder_get_object(bBook, "gDay"));
    gMonth = GTK_SPIN_BUTTON(gtk_builder_get_object(bBook, "gMonth"));
    gYear = GTK_SPIN_BUTTON(gtk_builder_get_object(bBook, "gYear"));
    ...
    gEmail = GTK_WIDGET(gtk_builder_get_object(bBook, "entry11"));
    ...
    gtk_window_set_title (GTK_WINDOW (wBook), "Записная книга 2");
