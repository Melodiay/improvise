...
enum
{
    C_ID, C_LAST_NAME, C_NAME, C_OTCHESTVO, C_NUMBER_TELEFONE, C_ADRES, C_PRIMECHANIE
};
...
int LoadSearchLN()
{
    ...
    char* SQL = "SELECT [Код контактов], Фамилия, Имя, Отчество, [Номер телефона], Адрес, Примечание \
...
void ShowSearch(int search)
{
    ...
    gtk_window_set_title (GTK_WINDOW (wSearch), "Записная книга");
