...
void loop(void) {
  ...
  **/
  ...
  if (termoprofily1_9 == 1){ 
    if (millis() >= myTimer1 + sec*1000) {   // таймер на 1000 мс (1 раза в 1 сек) автор таймера https://alexgyver.ru/lessons/
      //myTimer1 = millis();
      termoprofily0_1_9();
...
void nigniye(){
  ...
  analogWrite(nigniy_1, NizPID(tempt2, tempust2, Kpn, Kin, Kdn, 0.10, 0, pwmust2)); 
  //delay(20); 
}
// Пид регулирование верхний нагреватель
void verhniy(){
     // (вход, установка, п, и, д, период в секундах, мин.выход, макс. выход)
     analogWrite(verhniy_1, VerhPID(tempt1, tempust1, Kpv, Kiv, Kdv, 0.10, 0, pwmust1)); 
     //delay(20);
}
/**
...
void AnalyseString(String incStr) {
  ...
  if (incStr.indexOf("pidvsave") >= 0) {     // Сохранение ПИД регулирование, П И Д составляющих в eeprom память по умолчанию
     EEPROM.put(7, kpv);
     EEPROM.put(12, kiv);
     EEPROM.put(17, kdv);
  } 
  if (incStr.indexOf("pidnsave") >= 0) {     // Сохранение ПИД регулирование, П И Д составляющих в eeprom память по умолчанию
     EEPROM.put(212, kpn);
     EEPROM.put(217, kin);
     EEPROM.put(222, kdn);
  } 
