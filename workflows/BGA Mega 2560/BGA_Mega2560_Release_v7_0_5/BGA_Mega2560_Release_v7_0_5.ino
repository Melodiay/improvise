...
byte coolvust1 = 0;
byte coolpust2 = 0;
float kpv = 140.75284; // ПИД регулирование порпорциональное   значение по умолчанию
float kiv = 10.71293; // ПИД регулирование интегральное       значение по умолчанию 
float kdv = 317.39011;  // ПИД регулирование дифферинциальное   значение по умолчанию
...
void pidCountrolN()
{
  ...
  {  // без детектора ноля
    ...
    analogWrite(nigniy_1, pid2.getResultTimer());  // отправляем на мосфет
  ...
  {                 // читаем с датчика температуру
    ...
    // (после вызова можно получать это значение как regulator.output)
    // обновление происходит по встроенному таймеру на millis()
    analogWrite(nigniy_1, dimmer[0] = pid2.getResultTimer());  // отправляем на мосфет
...
void pidCountrolV()
{
  ...
  } else
  {
    ...
    }else if(tempt1 < tempust1)
    {
      ...
      analogWrite(verhniy_1, dimmer[1] = pid.getResultTimer());  // отправляем на мосфет
      // .getResultTimer() по сути возвращает regulator.output
