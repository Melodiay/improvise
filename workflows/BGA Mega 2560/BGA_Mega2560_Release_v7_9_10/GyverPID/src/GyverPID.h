...
public:
    ...
    // kp, ki, kd, dt
    GyverPID(float new_kp, float new_ki, float new_kd, float new_window, int16_t new_dt = 100) {
        setDt(new_dt);
        Kp = new_kp;
        Ki = new_ki;
        Kd = new_kd;
    ...
    datatype getResult() {
        ...
        datatype delta_input = (abs(prevInput - input) < window);	// изменение входного сигнала за dt
